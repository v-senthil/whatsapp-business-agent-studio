...
import { D as i, H as a, K as o, U as s, a as c, b as l, c as u, f as d, v as f, w as p, y as m } from "./chunk-CSCIHK7Q-CHXp-B5u.js";
import { t as h } from "./ordinal-95c16pGC.js";
import { t as g } from "./defaultLocale-aSMWOJHq.js";
import { i as _ } from "./chunk-5ZQYHXKU-Ckg5JQ60.js";
import { t as v } from "./chunk-WU5MYG2G-yvpvq9O9.js";
import { t as y } from "./chunk-4BX2VUAB-j_w7t4i7.js";
import { t as b } from "./chunk-2J33WTMH-CCkbVqsI.js";
import { i as x, n as S } from "./chunk-NZK2D7GU-vzBS4EZW.js";
...
var oe = /* @__PURE__ */ n((e, t) => {
	...
	for (let n of e.TreemapRows ?? []) {
		let e = n.item;
		if (!e) continue;
		let i = n.indent ? parseInt(n.indent) : 0, a = se(e), o = e.classSelector ? t.getStylesForClass(e.classSelector) : [], s = o.length > 0 ? o : void 0, c = {
			level: i,
			name: a,
			type: e.$type,
			value: e.value,
	...
	let i = X(r), a = /* @__PURE__ */ n((e, n) => {
		for (let r of e) t.addNode(r, n), r.children && r.children.length > 0 && a(r.children, n + 1);
	}, "addNodesRecursively");
	a(i, 0);
}, "populate"), se = /* @__PURE__ */ n((e) => e.name ? String(e.name) : "", "getItemName"), Z = {
	parser: { yy: void 0 },
	parse: /* @__PURE__ */ n(async (n) => {
		try {
			let r = await e("treemap", n);
	...
	draw: /* @__PURE__ */ n((e, i, a, o) => {
		let s = o.db, c = s.getConfig(), d = c.padding ?? ce, f = s.getDiagramTitle(), p = s.getRoot(), { themeVariables: m } = l();
		if (!p) return;
		let _ = f ? 30 : 0, y = v(i), S = c.nodeWidth ? c.nodeWidth * Q : 960, C = c.nodeHeight ? c.nodeHeight * Q : 500, w = S, T = C + _;
		y.attr("viewBox", `0 0 ${w} ${T}`), u(y, T, w, c.useMaxWidth);
		let E;
		try {
			let e = c.valueFormat || ",";
			if (e === "$0,0") E = /* @__PURE__ */ n((e) => "$" + g(",")(e), "valueFormat");
			else if (e.startsWith("$") && e.includes(",")) {
				let t = /\.\d+/.exec(e), r = t ? t[0] : "";
				E = /* @__PURE__ */ n((e) => "$" + g("," + r)(e), "valueFormat");
			} else if (e.startsWith("$")) {
				let t = e.substring(1);
				E = /* @__PURE__ */ n((e) => "$" + g(t || "")(e), "valueFormat");
			} else E = g(e);
		} catch (e) {
			t.error("Error creating format function:", e), E = g(",");
		}
		let D = h().range([
			"transparent",
			m.cScale0,
		...
		f && y.append("text").attr("x", w / 2).attr("y", _ / 2).attr("class", "treemapTitle").attr("text-anchor", "middle").attr("dominant-baseline", "middle").text(f);
		let A = y.append("g").attr("transform", `translate(0, ${_})`).attr("class", "treemapContainer"), j = R(p).sum((e) => e.value ?? 0).sort((e, t) => (t.value ?? 0) - (e.value ?? 0)), M = ae().size([S, C]).paddingTop((e) => e.children && e.children.length > 0 ? $ + Q : 0).paddingInner(d).paddingLeft((e) => e.children && e.children.length > 0 ? Q : 0).paddingRight((e) => e.children && e.children.length > 0 ? Q : 0).paddingBottom((e) => e.children && e.children.length > 0 ? Q : 0).round(!0)(j), N = M.descendants().filter((e) => e.children && e.children.length > 0), P = A.selectAll(".treemapSection").data(N).enter().append("g").attr("class", "treemapSection").attr("transform", (e) => `translate(${e.x0},${e.y0})`);
		...
		}), c.showValues !== !1 && I.append("text").attr("class", "treemapValue").attr("x", (e) => (e.x1 - e.x0) / 2).attr("y", function(e) {
			return (e.y1 - e.y0) / 2;
		}).attr("style", (e) => "text-anchor: middle; dominant-baseline: hanging; font-size: 28px;fill:" + k(e.data.name) + ";" + x({ cssCompiledStyles: e.data.cssCompiledStyles }).labelStyles.replace("color:", "fill:")).attr("clip-path", (e, t) => `url(#clip-${i}-${t})`).text((e) => e.value ? E(e.value) : "").each(function(e) {
		...
		}), b(y, c.diagramPadding ?? 8, "flowchart", c?.useMaxWidth || !1);
	}, "draw"),
	...
	styles: /* @__PURE__ */ n(({ treemap: e } = {}) => {
		let t = _(i(), l().themeVariables), n = _(ue, e), r = n.titleColor ?? t.titleColor, a = n.labelColor ?? t.textColor, o = n.valueColor ?? t.textColor;
		return `
  .treemapNode.section {
