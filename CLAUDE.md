...
5. **If the build ever fails on first run** with `PageNotFoundError: /_document`, re-run `npm run build`. It was a flake on Next 15.1.4 and appears fixed since 15.5.x, but the retry is harmless.
...
**EntityPicker sibling phones.** Header's `<EntityPicker>` (`src/components/shell/EntityPicker.tsx`) opens a popover that lists other phones on the current WABA so the user can jump across without going back to `/home`. Under the hood, `usePhoneDetails(entityId)` returns `whatsapp_business_account.id` (the Graph phone route now fetches `whatsapp_business_account{id,name}` — see `src/app/api/graph/phone/route.ts`), then `usePhones(wabaId)` fills the list. The sibling query only runs when the popover is open (`enabled: open ? wabaId : undefined`) so the header does not spam the Graph API on every dashboard mount.
...
- `src/components/dev/DevDrawer.tsx` — slide-in panel with a call list + detail pane. Opens via the **Dev drawer** item in the Header settings dropdown (with a check mark when open), `Cmd/Ctrl+Shift+D`, or `?debug=1`. The old floating bottom-right button was removed since the settings menu now covers the same entry point.
...
1. `unset NODE_ENV` (do NOT re-add `export NODE_ENV=development`). Setting it to any non-empty value other than `production` before `next build` has historically produced `ENOENT: .next/server/pages-manifest.json` on some patch versions; leaving it unset lets Next set it to `production` internally. Dev-deps are still installed because the install step uses `--include=dev`.
