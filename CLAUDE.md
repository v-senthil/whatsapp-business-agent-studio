...
**Eligibility-first gate.** `<EntityGate>` (`src/components/shell/EntityGate.tsx`) wraps every child of the entity layout (`src/app/dashboard/[entityId]/layout.tsx`). It runs `useEligibility(entityId)` and only renders `{children}` after the request resolves successfully. While pending it shows a skeleton; on TOS-403 it shows a short placeholder (the banner + sidebar already carry the block message); on other errors it renders `<ErrorState>`. This means downstream hooks (`useSettings`, `useSkills`, `useConnectors`, etc.) never fire until we know the phone can host an agent — every other Meta Agent Platform call is short-circuited when the browser is still checking eligibility or when eligibility failed.
