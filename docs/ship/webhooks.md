...
- The live SSE stream is filtered server-side to the phone number the browser is currently viewing. Events whose `phone_number_id` matches, plus events that carry no `phone_number_id` at all (test payloads, non-message changes), are pushed to that tab. Events for other phones never leave the server.
